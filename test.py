...
def get_name_of_class(clazz, class_index: int) -> str:
    return clazz["constant_pool"][
        clazz["constant_pool"][class_index - 1]["name_index"] - 1
    ]["bytes"].decode("utf-8")


def get_name_of_member(clazz, name_and_type_index: int) -> str:
    return clazz["constant_pool"][
        clazz["constant_pool"][name_and_type_index - 1]["name_index"] - 1
    ]["bytes"].decode("utf-8")
    ...
    with io.BytesIO(code) as f:
        while f.tell() < len(code):
            ...
            if opcode == getstatic_opcode:
                ...
                name_of_member = get_name_of_member(
                ...
                if (
                    name_of_class == "java/io/PrintStream"
                    and name_of_member == "println"
                ):
                    n = len(stack)
                    if len(stack) < 2:
                        raise RuntimeError(
                            "{name_of_class}/{name_of_member} expectes 2 arguments, but provided {n}"
                        )
